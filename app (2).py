...
UNET_GDRIVE_LINK = "https://drive.google.com/file/d/1u5c1QAr9Z9x1JA9nlxlLGHub2dRP6Xpc/view?usp=sharing"

# File names to save as locally (change if your file has a different name)
SAM_FILENAME  = "sam_vit_b_01ec64.pth"   # always a .pth
UNET_FILENAME = "unet_foot.zip"           # set to .pth if yours is not zipped
...
def resolve_model(link, filename, label):
    ...
    try:
        ...
        # ── If it's a zip, extract and find the .pth inside
        if filename.endswith(".zip"):
            extract_dir = os.path.join(MODELS_DIR, filename.replace(".zip", "_extracted"))
            os.makedirs(extract_dir, exist_ok=True)
            print(f"[{label}] Extracting zip to: {extract_dir}")
            with zipfile.ZipFile(dest, "r") as zf:
                zf.extractall(extract_dir)
            os.remove(dest)  # remove zip to save space

            # Walk the extracted directory and find first .pth
            for root, _, files in os.walk(extract_dir):
                for f in sorted(files):
                    print(f"[{label}] Found extracted file: {os.path.join(root, f)}")
                    if f.endswith(".pth"):
                        found = os.path.join(root, f)
                        print(f"[{label}] ✅ Using: {found}")
                        return found

            print(f"[{label}] ❌ No .pth file found inside zip.")
            st.error(f"❌ {label}: No .pth found inside the zip. Check its contents.")
            return None
...
def load_pipeline(sam_ckpt, unet_ckpt):
    ...
    print(f"[Pipeline] Loading SAM  from: {sam_ckpt}")
    print(f"[Pipeline] Loading UNet from: {unet_ckpt}")
    pipe = ShoeSizePipeline(
    ...
    print(f"[Pipeline] SAM  loaded: {pipe.sam_predictor is not None}")
    print(f"[Pipeline] UNet loaded: {pipe.unet_model is not None}")
    return pipe
