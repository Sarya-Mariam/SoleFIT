...
class ShoeSizePipeline:
            ...
            try:
                ...
                self.unet_model.load_state_dict(
                    torch.load(unet_ckpt_path, map_location=self.device,
                               weights_only=True)   # safer + less memory
                )
    ...
    def predict(self, image_bgr, mode="ceil", jack_purcell=False, unet_thresh=0.5):
        ...
        # Downscale large images before processing — saves significant RAM
        h0, w0 = image_bgr.shape[:2]
        MAX_DIM = 1920
        if max(h0, w0) > MAX_DIM:
            ...
            image_bgr = cv2.resize(image_bgr,
                                   ...
                                   interpolation=cv2.INTER_AREA)
            print(f"[predict] Resized input from {w0}×{h0} to {image_bgr.shape[1]}×{image_bgr.shape[0]}")

        # Step 1 — A4 detection
        print("[predict] Step 1: A4 detection")
        ...
        print("[predict] Step 2: UNet foot segmentation")
        foot_mask = unet_segment_foot_fullres(
            image_bgr, self.unet_model, self.device, thresh=unet_thresh
        )
        free_memory()

        # Step 3 — Foot length from A4 bottom edge
        print("[predict] Step 3: Measuring foot length")
        ...
        print(f"[predict] Size: {sizes_rec}")

        # Step 5 — Visualise
        print("[predict] Step 4: Drawing results")
